...
// cva not needed here — variants handled manually
...
type ToastVariantProps = { variant?: "default" | "success" | "warning" | "error" | "info"; className?: string };

function toastVariantsFn(props?: ToastVariantProps) {
  return cn(
    "group pointer-events-auto relative flex w-full items-start space-x-3 overflow-hidden rounded-xl border p-4 pr-8 shadow-lg transition-all",
    "data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none",
    "data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out",
    "data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full",
    "data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
    props?.variant === "success" && "border-emerald-500/30 bg-emerald-500/10 text-emerald-100",
    props?.variant === "warning" && "border-amber-500/30 bg-amber-500/10 text-amber-100",
    props?.variant === "error" && "border-red-500/30 bg-red-500/10 text-red-100",
    props?.variant === "info" && "border-blue-500/30 bg-blue-500/10 text-blue-100",
    (!props?.variant || props.variant === "default") && "border-border bg-card text-card-foreground",
    props?.className
  );
}
...
>(({ className, variant = "default", ...props }, ref) => {
  return (
    <ToastPrimitive.Root
      ref={ref}
      className={toastVariantsFn({ variant, className })}
      {...props}
    >
      {TOAST_ICONS[variant]}
